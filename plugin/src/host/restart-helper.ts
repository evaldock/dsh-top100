...
process.once("message", async (launch: { pid: number; file: string; args: string[]; cwd: string; port: number; env: NodeJS.ProcessEnv; trace: string }) => {
  // Only stage codes and process ids, never argv, environment, auth URLs or host output.
  const note = (stage: string, childPid?: number, exitCode?: number | null) => {
    try { writeFileSync(launch.trace, JSON.stringify({ stage, parentPid: launch.pid, childPid, exitCode, at: new Date().toISOString() }), { mode: 0o600 }); } catch { /* best effort */ }
  };
  clearTimeout(waiting);
  note("waiting-for-exit");
  process.send?.("armed");
  const deadline = Date.now() + 30000;
  while (Date.now() < deadline && (alive(launch.pid) || await listening(launch.port))) await sleep(200);
  // Never spawn a duplicate if the old process refused to stop.
  if (alive(launch.pid) || await listening(launch.port)) { note("old-host-still-running"); process.exitCode = 1; return; }
  await sleep(300);
