...
export function formatRecommendationResult(result) {
    ...
    for (const [index, item] of result.items.entries()) {
        ...
        lines.push(`${index + 1}. ${item.fullName} — ${item.description}`, `   类型：${item.type}；分类：${category}；Stars：${item.stars}；日增：${item.dailyStars ?? "未提供"}；周增：${item.weeklyStars ?? "未提供"}；${install}`, `   形态：${item.formFactor}；信任层：${item.trustLevel}；证据：${item.trustSignals.join("、")}`, `   注意：${item.trustCaveat}`, `   ${item.repositoryUrl}`);
    }
    return lines.join("\n");
}
