...
export function trustedRestartRequest(request, requireOrigin = true) {
    if (!["127.0.0.1", "::1", "::ffff:127.0.0.1"].includes(request.socket?.remoteAddress ?? ""))
        return false;
    if (Object.keys(request.headers).some((key) => key === "forwarded" || key.startsWith("x-forwarded-") || key === "x-real-ip"))
        return false;
...
export function restartCapability(options) {
    ...
    if (!options.currentProfile)
        return no("profile");
    if (options.allowRestart === false || env.DSH_TOP100_ALLOW_RESTART === "0")
        return no("disabled");
    // A managed service must keep its process ownership; never guess its restart command.
    let systemdParent = process.ppid === 1;
    try {
        systemdParent ||= readFileSync(`/proc/${process.ppid}/comm`, "utf8").trim() === "systemd";
    }
    catch { /* non-Linux */ }
    if (((env.INVOCATION_ID || env.JOURNAL_STREAM) && systemdParent) || env.pm_id || env.PM2_HOME || env.SUPERVISOR_ENABLED || env.LAUNCH_JOBKEY)
        return no("supervised");
    const flags = [...(options.execArgv ?? process.execArgv), ...(env.NODE_OPTIONS ?? "").split(/\s+/)];
    if ((options.inspectorUrl === undefined ? inspector.url() : options.inspectorUrl) || flags.some((flag) => /^--(?:inspect|debug)(?:[-=]|$)/.test(flag)))
        return no("debugger");
    const argv = options.argv ?? process.argv;
    if (!argv[1] || !/[\\/](?:bin\.(?:js|ts)|dsh)$/.test(argv[1]) || !argv.includes("web") || !existsSync(resolve(argv[1])))
        return no("launcher");
...
export function prepareRestart(port) {
    if (!Number.isInteger(port) || port < 1 || port > 65535)
        return Promise.reject(new Error("invalid restart port"));
                    ...
                    return new Promise((done, fail) => {
                        ...
                        helper.send({ pid: process.pid, file: process.execPath, args, cwd: process.cwd(), port, env: process.env, trace }, (error) => {
                            if (error) {
                                clearTimeout(timer);
                                helper.kill();
                                fail(new Error("restart handoff failed"));
                            }
                        });
                    });
                },
